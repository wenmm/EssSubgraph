...
def buildModel(dataDict, numFeat, numberOfClasses, f_tp, f_fp, f_th, expName, iteration, model_dir, result_dir):
    ...
    prauc = auc(r0, p0) #new
    prauc0 = os.path.join(result_dir, expName + '_prauc.txt')
    roc_auc0 = os.path.join(result_dir, expName + '_auc.txt')
    ...
    with open(prauc0, 'a') as f0:
        f0.write(str(prauc) + '\t')
        f0.write('\n')

    with open(roc_auc0, 'a') as f0:
        f0.write(str(roc_auc) + '\t')
        f0.write('\n')
