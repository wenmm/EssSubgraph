...
for curr_labels in labels:
        ...
        # Remove nodes from labels not in `self.union`
            labels1 = {node: labels_ for node, labels_ in curr_labels.items() if node in union}
            labels_mh = labels1.values()
            labels_mh = pd.DataFrame(labels_mh, index=labels1.keys())
...
for i in range(10):
    for cv_run in range(5):
        ...
        #node_sets = [np.union1d(G[0].values, G[1].values) for G in graphs]
        #union = reduce(np.union1d, node_sets)
        # mapper = {node: idx for idx, node in enumerate(union)}
        ...
        # e_idx = [mapper[i] for i in e_lst if i in mapper and i in labels[0]]
        # ne_idx = [mapper[i] for i in ne_lst if i in mapper  and i in labels[0]]
        #print("train_valid_idx_list size:", len(train_valid_idx_list))
        #print("First few train_valid_gnames:", list(train_valid_gnames)[:5])
        ...
        #     #all_scaled = scaler.transform(all_data)
            if len(train_valid_pca_data) != 0:
                 reduced_matrix = pca.fit_transform(train_valid_pca_data)
        #         print(len(train_valid_pca_data))
                ...
                # Create PyG Data object
                 ...
                 pyg_graph.train_mask = k_sets_net[i][cv_run][0]
                 pyg_graph.valid_mask = k_sets_net[i][cv_run][1]
                 pyg_graph.test_mask = k_sets_net[i][cv_run][2]
...
import pickle
with open('test_cancer_full_expression_pc50_PCA_exclude_test_{}.pkl'.format(network_name), 'wb') as f:
