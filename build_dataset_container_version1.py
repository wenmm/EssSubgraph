...
for i in range(10):
    for cv_run in range(5):
        ...
        node_sets = [np.union1d(G[0].values, G[1].values)]
        union = reduce(np.union1d, node_sets)
        ...
        for k in np.arange(0,10): # Randomly generate 5CV splits for ten times
            ...
            for cv_idx in np.arange(1,6):
                ...
                train_mask = a[:int(len(cv["train_1"])/10*9)]
                valid_mask = a[int(len(cv["train_1"])/10*9):]
                k_sets_net_pyg[k].append((train_mask, valid_mask, test_mask))
                n += 1
...
import pickle
with open('test_cancer_full_expression_pc50_G_exclude_test_{}.pkl'.format(network_name), 'wb') as f:
